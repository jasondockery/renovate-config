...
test('every executable workflow keeps one sanitized receipt and explicit retention contract', () => {
  ...
  assert.match(ci, /--output compass-hosted-adoption-receipt\.json/)
  assert.match(ci, /--workflow \.github\/workflows\/ci\.yml/)
  ...
  assert.match(ci, /\[ "\$HEAD_SHA" = "\$GITHUB_SHA" \]/)
  ...
  assert.match(renovate, /mktemp -d "\$RUNNER_TEMP\/renovate-runtime-\$\{GITHUB_RUN_ID\}-\$\{GITHUB_RUN_ATTEMPT\}\.XXXXXX"/)
  assert.match(renovate, /chmod 0700 "\$log_dir"/)
  assert.match(renovate, /chmod 0700 "\$runtime_dir"/)
  assert.match(renovate, /stat -Lc '%d:%i' "\$log_dir"/)
  assert.match(renovate, /stat -Lc '%d:%i' "\$runtime_dir"/)
  ...
  assert.match(renovate, /printf 'phase-file=%s\\n' "\$phase_file" >> "\$GITHUB_OUTPUT"/)
  assert.match(renovate, /printf 'summary-file=%s\\n' "\$summary_file" >> "\$GITHUB_OUTPUT"/)
  ...
  assert.match(renovate, /--phase-file "\$RENOVATE_PHASE_FILE"/)
  assert.match(renovate, /--summary "\$RENOVATE_SUMMARY_FILE"/)
  assert.doesNotMatch(renovate, /\$RUNNER_TEMP\/renovate-(?:runner\.json|container-entrypoint\.sh|phases\.tsv|summary\.md)/)
  assert.match(renovate, /Renovate runtime cleanup target identity changed/)
  assert.match(renovate, /Renovate runtime cleanup paths escaped the captured directory/)
  assert.match(renovate, /rm -f -- "\$file"/)
  assert.match(renovate, /rmdir -- "\$RENOVATE_RUNTIME_DIR"/)

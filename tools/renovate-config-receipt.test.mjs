...
test('every executable workflow keeps one sanitized receipt and explicit retention contract', () => {
  ...
  assert.match(renovate, /LOG_FILE: \/tmp\/renovate-run-/)
  ...
  assert.match(renovate, /receipt producer consumes the raw debug log/)
  assert.match(renovate, /Raw Renovate log cleanup fallback used/)
  assert.match(renovate, /Raw Renovate log cleanup failed/)

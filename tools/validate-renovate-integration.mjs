...
const completed = spawnSync(
  'npx',
  ['--yes', '--package', `renovate@${version}`, '--', 'node', 'tools/run-renovate-integration.mjs'],
  {
    cwd: process.cwd(),
    env: process.env,
    stdio: 'inherit',
...
if (completed.error) {
  console.error(`renovate integration could not acquire Renovate ${version}: ${completed.error.message}`)
  process.exitCode = 1
} else {

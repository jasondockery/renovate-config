...
Next action: land the owner-approved five-day policy and receipt compatibility,
then accept the exact live run and resulting consumer pull requests
...
| 4 | renovate-config thesis, dependency schema, acceptance contracts, extraction/scanner/audit/receipt tooling, and **manual-only** compatibility workflow; exclude the then-owner-gated preset policy files and package script | `pnpm verify --report /tmp/renovate-config-tooling.json` | renovate-config exact-SHA CI green |
...
| 8 | Owner-approved preset exception only: exception spec, reviewed fixture, effective-policy checker/command, reviewed `default.json`, and freeze checksum | `pnpm verify --report /tmp/renovate-config-preset.json`; separately run `pnpm renovate:policy` as the non-duplicated effective-policy field proof | renovate-config exact-SHA CI green, owner acceptance recorded, and live receipt green |
...
and stage those hunks independently. Row 8 is the only authorized exception
that changes the frozen preset checksum. The new checksum becomes the immutable
baseline until versioned distribution.
